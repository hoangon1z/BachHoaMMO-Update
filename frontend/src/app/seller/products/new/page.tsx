...
import { ArrowLeft, Save, Image, Plus, X, Layers, Info, Package, ArrowUpCircle, Mail, AlertCircle, CheckCircle, Globe, Upload, Loader2, MonitorUp, Zap } from 'lucide-react';
...
type ProductType = 'STANDARD' | 'UPGRADE' | 'SERVICE';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requestBody: any = {
        ...
        autoDelivery: productType === 'UPGRADE' || productType === 'SERVICE' ? false : autoDelivery,
        productType,
        requiredBuyerFields: (productType === 'UPGRADE' || productType === 'SERVICE') ? JSON.stringify(requiredBuyerFields) : undefined,
      };
      ...
      if (hasVariants) {
        ...
        requestBody.originalPrice = parsePriceValue(formData.originalPrice) ? parseFloat(parsePriceValue(formData.originalPrice)) : undefined;
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <button
                ...
                className={`p-4 rounded-xl border-2 text-left transition-all ${productType === 'STANDARD'
                  ? 'border-blue-500 bg-blue-50'
                  : 'border-gray-200 hover:border-gray-300'
                  }`}
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${productType === 'STANDARD' ? 'bg-blue-100' : 'bg-gray-100'
                    }`}>
              ...
              <button
                ...
                className={`p-4 rounded-xl border-2 text-left transition-all ${productType === 'UPGRADE'
                  ? 'border-purple-500 bg-purple-50'
                  : 'border-gray-200 hover:border-gray-300'
                  }`}
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${productType === 'UPGRADE' ? 'bg-purple-100' : 'bg-gray-100'
                    }`}>
              ...
              <button
                ...
                className={`p-4 rounded-xl border-2 text-left transition-all ${productType === 'SERVICE'
                  ? 'border-emerald-500 bg-emerald-50'
                  : 'border-gray-200 hover:border-gray-300'
                  }`}
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${productType === 'SERVICE' ? 'bg-emerald-100' : 'bg-gray-100'
                    }`}>
                    <Zap className={`w-5 h-5 ${productType === 'SERVICE' ? 'text-emerald-600' : 'text-gray-500'}`} />
                  </div>
                  <div>
                    <span className={`font-semibold ${productType === 'SERVICE' ? 'text-emerald-700' : 'text-gray-700'}`}>
                      Dịch vụ MXH
                    </span>
                ...
                <p className="text-sm text-gray-600">
                  Dịch vụ tăng like, follow, view, sub... Khách nhập link + số lượng, bạn xử lý.
                </p>
              </button>
            </div>
            ...
            {productType === 'SERVICE' && (
              <div className="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {[{ key: 'link', label: 'Link/URL' }, { key: 'username', label: 'Tên tài khoản' }, { key: 'email', label: 'Email' }, { key: 'password', label: 'Mật khẩu' }, { key: 'quantity', label: 'Số lượng' }, { key: 'note', label: 'Ghi chú' }].map((field) => (
                    <label
                      key={field.key}
                      className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-all ${requiredBuyerFields.includes(field.key)
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={requiredBuyerFields.includes(field.key)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setRequiredBuyerFields([...requiredBuyerFields, field.key]);
                          } else {
                            setRequiredBuyerFields(requiredBuyerFields.filter(f => f !== field.key));
                          }
                        }}
                        className="sr-only"
                      />
              ...
              <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {['email', 'password', 'username'].map((field) => (
                    <label
                      key={field}
                      className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-all ${requiredBuyerFields.includes(field)
